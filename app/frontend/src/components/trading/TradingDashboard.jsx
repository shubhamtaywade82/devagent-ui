...
import { TrendingUp, TrendingDown, Wallet, DollarSign, Activity, RefreshCw } from 'lucide-react'
...
function TradingDashboard({ accessToken }) {
  ...
  const [errors, setErrors] = useState({ funds: null, positions: null, orders: null })
  ...
  const loadDashboardData = async () => {
    ...
    try {
      const [fundsData, positionsData, ordersData] = await Promise.all([
        api.getFunds(accessToken).catch(err => ({ success: false, error: err.message })),
        api.getPositions(accessToken).catch(err => ({ success: false, error: err.message })),
        api.getOrders(accessToken).catch(err => ({ success: false, error: err.message }))
      ])
      ...
      if (positionsData.success) {
        const positionsArray = Array.isArray(positionsData.data)
          ? positionsData.data
          : (positionsData.data?.data || positionsData.data?.positions || positionsData.data?.position || [])
        setPositions(positionsArray)
        setErrors(prev => ({ ...prev, positions: null }))
        console.log('Positions data:', positionsArray)
      } else {
        setErrors(prev => ({ ...prev, positions: positionsData.error || 'Failed to load positions' }))
      ...
      if (ordersData.success) {
        const ordersArray = Array.isArray(ordersData.data)
          ? ordersData.data
          : (ordersData.data?.data || ordersData.data?.orders || ordersData.data?.order || [])
        setOrders(ordersArray)
        setErrors(prev => ({ ...prev, orders: null }))
        console.log('Orders data:', ordersArray)
      } else {
        setErrors(prev => ({ ...prev, orders: ordersData.error || 'Failed to load orders' }))
        console.error('Orders error:', ordersData.error)
      }
    } catch (error) {
      console.error('Failed to load dashboard data:', error)
      setErrors({ funds: error.message, positions: error.message, orders: error.message })
    } finally {
      setLoading(false)
    }
  }
  ...
  return (
    <div className="h-full overflow-y-auto p-6 bg-zinc-950">
          ...
          {errors.funds && (
            <div className="p-3 bg-red-500/20 border border-red-500/50 rounded-lg text-red-400 text-sm">
              ...
              <div className="text-xs text-red-300 mt-1">
                This might indicate an authentication issue or API connectivity problem.
              </div>
            </div>
          )}
          {errors.positions && (
      ...
      {!hasErrors && !loading && funds && availableBalance === 0 && marginUsed === 0 && positions.length === 0 && orders.length === 0 && (
        <div className="mb-4 p-4 bg-blue-500/20 border border-blue-500/50 rounded-lg text-blue-400 text-sm">
          <strong>New Account Detected:</strong> Your account appears to be new or empty. This is normal for demo accounts or newly created trading accounts.
          Start by placing an order or check your account balance in the DhanHQ portal.
        </div>
      )}
      ...
      {process.env.NODE_ENV === 'development' && funds && (
        <div className="mb-4 p-3 bg-zinc-900/50 border border-zinc-800 rounded-lg text-xs text-zinc-500">
          <details>
            <summary className="cursor-pointer text-zinc-400">Debug: Funds Data Structure</summary>
            <pre className="mt-2 overflow-auto max-h-40">{JSON.stringify(funds, null, 2)}</pre>
          </details>
        </div>
      )}
      ...
      <div className="glass rounded-lg p-6 mb-6">
        ...
        {errors.positions ? (
          <div className="text-center text-red-400 py-8">
            {errors.positions}
            <div className="text-sm text-zinc-500 mt-2">Check console for details</div>
          </div>
        ) : !Array.isArray(positions) || positions.length === 0 ? (
          <div className="text-center text-zinc-500 py-8">
            <div className="mb-2">No open positions</div>
            <div className="text-xs text-zinc-600">Open positions will appear here when you have active trades</div>
          </div>
        ) : (
      ...
      <div className="glass rounded-lg p-6">
        ...
        {errors.orders ? (
          ...
          <div className="text-center text-zinc-500 py-8">
            ...
            <div className="text-xs text-zinc-600">Your order history will appear here after placing orders</div>
          </div>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full">
